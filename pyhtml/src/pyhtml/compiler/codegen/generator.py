...
class CodeGenerator:
    ...
    def generate(self, parsed: ParsedPyHTML) -> ast.Module:
        ...
        page_class = self._generate_page_class(
            parsed, handlers, known_methods, known_vars, known_imports, async_methods
        )
    ...
    def _generate_page_class(
        ...
        render_func, binding_funcs = self._generate_render_template_method(
            parsed, known_methods, known_imports, async_methods
        )
        if render_func:
    ...
    def _transform_inline_code(
        self, code: str, known_methods: Set[str] = None, async_methods: Set[str] = None
                    ...
                    value=ast.Call(
                        ...
                        args=[
                            ...
                            ast.Dict(
                                ...
                                + [ast.Constant(value=name) for name in known_globals],
    ...
    def _transform_user_code(
        self, python_ast: ast.Module, known_globals: Set[str] = None
    ) -> List[ast.stmt]:
    ...
    def _generate_render_template_method(
    ...
    ) -> Tuple[Optional[ast.FunctionDef], List[ast.AsyncFunctionDef]]:
        ...
        if layout_directive:
            ...
            slot_funcs_methods, aux_funcs = self.template_codegen.generate_slot_methods(
                parsed.template, file_id=file_id, known_globals=known_globals, layout_id=layout_id
            )
