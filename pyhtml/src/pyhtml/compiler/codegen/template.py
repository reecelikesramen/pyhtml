    ...
    def generate_slot_methods(
        ...
        for slot_name, nodes in slots.items():
            ...
            slot_funcs[slot_name] = self._generate_function(
                nodes, func_name, is_async=True, known_globals=known_globals, layout_id=layout_id
            )
        ...
        class AddSelfTransformer(ast.NodeTransformer):
            def visit_Name(self, node):
                ...
                if (
                    ...
                    and (known_globals is None or node.id not in known_globals)
                ):
    ...
    def _add_node(
        ...
        if node.tag is None:
            ...
            attr_loop = ast.For(
                ...
                iter=ast.Call(
                    func=ast.Attribute(
                        value=ast.Name(id="attrs", ctx=ast.Load()), attr="items", ctx=ast.Load()
                    ),
                    args=[],
                    keywords=[],
                ),
                            ...
                            args=[
                                ast.JoinedStr(
                                    values=[
                                        ast.Constant(value=" "),
                                        ast.FormattedValue(
                                            value=ast.Name(id="k", ctx=ast.Load()), conversion=-1
                                        ),
                                        ast.Constant(value='="'),
                                        ast.FormattedValue(
                                            value=ast.Name(id="val", ctx=ast.Load()), conversion=-1
                                        ),
                                        ast.Constant(value='"'),
                                    ]
                                )
