...
class FormValidator:
    ...
    def validate_field(
        ...
        if is_required:
            if value is None or (isinstance(value, str) and value.strip() == ''):
                return rules.title or f"This field is required"
        ...
        if rules.pattern:
            try:
                if not re.fullmatch(rules.pattern, str_value):
                    return rules.title or f"Value does not match the required pattern"
            except re.error:
    ...
    def validate_form(self, data: Dict[str, Any], schema: Dict[str, FieldRules], state_getter: Callable[[str], Any]) -> Tuple[Dict[str, Any], Dict[str, str]]:
        """
        ...
        for field_name, rules in schema.items():
            ...
            try:
                ...
                errors[field_name] = f"{rules.title or field_name} must be a valid {rules.input_type}."
                continue

            # Validate rules against converted value
            error = self.validate_field(field_name, converted_value, rules, state_getter) # Use original state_getter for validate_field
            if error:
