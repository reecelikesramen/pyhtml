...
async def run_dev_server(
    ...
    async def watch_changes():
        try:
            ...
            if app_module_path:
                files_to_watch.append(
                    app_module_path.parent
                )  # Watch the dir containing main.py? Or just the file? awatch takes paths.
    ...
    async with asyncio.TaskGroup() as tg:
        if HAS_HTTP3:
            try:
                ...
                print(
                    f"PyHTML: Running with Hypercorn (HTTP/3 + WebSocket) on https://{display_host}:{port}"
                )
