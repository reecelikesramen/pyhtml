...
class TestAppExhaustive(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.ws_patcher = patch('pyhtml.runtime.app.WebSocketHandler')
        self.mock_ws = self.ws_patcher.start()
        
        self.http_patcher = patch('pyhtml.runtime.app.HTTPTransportHandler')
        self.mock_http = self.http_patcher.start()
        
        self.wt_patcher = patch('pyhtml.runtime.webtransport_handler.WebTransportHandler')
    ...
    def test_load_pages_recursive(self):
        # Create a nested structure
        (self.pages_dir / "sub").mkdir()
        (self.pages_dir / "index.pyhtml").touch()
        (self.pages_dir / "about.pyhtml").touch()
        (self.pages_dir / "sub" / "contact.pyhtml").touch()
        (self.pages_dir / "sub" / "[id].pyhtml").touch()
    ...
    async def _async_test_upload(self, app, token, content_length=100, files=None):
    ...
    def test_scan_directory_complex(self):
        # 1. Hidden file
        (self.pages_dir / "_hidden.pyhtml").touch()
        # 2. Param directory
        (self.pages_dir / "[user_id]").mkdir()
        (self.pages_dir / "[user_id]" / "profile.pyhtml").touch()
        # 3. Trailing slash case (index in sub)
        (self.pages_dir / "about").mkdir()
        (self.pages_dir / "about" / "index.pyhtml").touch()
        # 4. Explicit !path routes
        (self.pages_dir / "custom.pyhtml").touch()
        
        class ExplicitPage(MockPage):
            __routes__ = {"alt": "/my-custom-path"}
            
        def mock_load(path, **kwargs):
            if "custom.pyhtml" in str(path):
                return ExplicitPage
            return MockPage
    ...
    def test_scan_directory_load_fail(self):
        (self.pages_dir / "broken.pyhtml").touch()
        # Fail load
    ...
    def test_handle_request_event_exception(self):
        ...
        with patch.object(MockPage, 'handle_event', new_callable=AsyncMock) as mock_handle:
            ...
            headers = {"X-PyHTML-Event": "click"}
            loop = asyncio.get_event_loop()
            response = loop.run_until_complete(self._async_test_request(app, method="POST", headers=headers))
            self.assertEqual(response.status_code, 500)
    ...
    def test_handle_upload_security(self):
        ...
        # 1. No token
        response = loop.run_until_complete(self._async_test_upload(app, None))
        ...
        response = loop.run_until_complete(self._async_test_upload(app, "valid_token", content_length=20*1024*1024))
    ...
    def test_reload_page(self):
        ...
        with patch.object(app.router, 'remove_routes_for_file') as mock_remove, \
    ...
    async def _async_test_request(self, app, method="GET", headers=None, path="/test", json_data=None):
        request = AsyncMock(spec=Request)
    ...
    def test_handle_request_event(self):
        ...
        with patch.object(MockPage, 'handle_event', new_callable=AsyncMock) as mock_handle:
            ...
            loop = asyncio.get_event_loop()
            response = loop.run_until_complete(self._async_test_request(app, method="POST", headers=headers, json_data=json_data))
    ...
    def test_handle_request_injection(self):
        ...
        with patch.object(MockPage, '__init__', mocked_init):
             ...
             body = response.body.decode()
             self.assertIn("window.PYHTML_CERT_HASH = [10, 20]", body)
    ...
    def test_asgi_call(self):
        ...
        # 1. WebTransport
        with patch.object(app.web_transport_handler, 'handle', new_callable=AsyncMock) as mock_wt_handle:
