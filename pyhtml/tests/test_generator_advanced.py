...
from pyhtml.compiler.ast_nodes import ParsedPyHTML, TemplateNode, LayoutDirective
...
class TestGeneratorAdvanced(unittest.TestCase):
    ...
    def test_generate_layout_mode(self):
        ...
        init_slots = next(n for n in class_def.body if isinstance(n, ast.FunctionDef) and n.name == "_init_slots")
        ...
        self.assertTrue(any(isinstance(n, ast.Expr) and isinstance(n.value, ast.Call) for n in init_slots.body))
    ...
    def test_generate_spa_metadata(self):
        ...
        parsed = ParsedPyHTML(template=[], directives=[path], python_code="", python_ast=ast.parse(""), file_path="p.pyhtml")
        
        stmts = self.generator._generate_spa_metadata(parsed)
        # __spa_enabled__ = True
        self.assertTrue(any(isinstance(s, ast.Assign) and s.targets[0].id == "__spa_enabled__" and s.value.value is True for s in stmts))
        # __sibling_paths__ = ['/a', '/b']
        self.assertTrue(any(isinstance(s, ast.Assign) and s.targets[0].id == "__sibling_paths__" and len(s.value.elts) == 2 for s in stmts))

    def test_generate_init_method(self):
        parsed = ParsedPyHTML(template=[], python_code="", python_ast=ast.parse(""), file_path="test.pyhtml")
        ...
        self.assertTrue(any(isinstance(n, ast.Expr) and isinstance(n.value, ast.Call) and n.value.func.attr == "__init__" for n in init_func.body))
        self.assertTrue(any(isinstance(n, ast.Expr) and isinstance(n.value, ast.Call) and n.value.func.attr == "_init_slots" for n in init_func.body))
