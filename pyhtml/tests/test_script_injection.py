...
from pyhtml.compiler.ast_nodes import ParsedPyHTML, TemplateNode, PathDirective
...
def test_bundle_selection_integration(tmp_path):
    ...
    pages_dir.mkdir()
    (pages_dir / "index.pyhtml").write_text("!path { 'a': '/a', 'b': '/b' }\n<h1>Index</h1>\n---\n# Python")
...
def test_no_spa_directive_disables_injection(tmp_path):
    pages_dir = tmp_path / "pages"
    pages_dir.mkdir()
    (pages_dir / "no_spa.pyhtml").write_text("!path { 'a': '/a', 'b': '/b' }\n!no_spa\n<h1>No SPA</h1>\n---\n# Python")
