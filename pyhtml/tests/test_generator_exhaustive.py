...
from pyhtml.compiler.ast_nodes import (
    ParsedPyHTML, TemplateNode, EventAttribute, FormValidationSchema, FieldValidationRules
)
...
class TestGeneratorExhaustive(unittest.TestCase):
    ...
    def test_process_handlers_busy_binding(self):
        ...
        node.special_attributes = [
            EventAttribute(name="@input", value="update", 
                           event_type="input", handler_name="update", 
                           line=1, column=0),
            BindAttribute(name="$bind", value="busy", variable="is_忙", binding_type="busy", line=1, column=0)
        ]
    ...
    def test_generate_form_validation_complex(self):
        ...
        schema = FormValidationSchema(fields={
            "email": FieldValidationRules(name="email", required=True)
        }, model_name="MyModel")
        ...
        wrapper = next(m for m in methods if isinstance(m, ast.AsyncFunctionDef) and m.name.startswith("_form_submit_"))
