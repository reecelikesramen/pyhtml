---
name: pywire-lsp
description: Guides development and modification of the PyWire Language Server. Use when working on LSP features like diagnostics, hover, completion, or go-to-definition for .pywire files.
---

# PyWire LSP Development

This skill provides guidance for modifying the PyWire Language Server Protocol (LSP) implementation.

## Project Structure

| Path | Description |
|------|-------------|
| `lsp/src/pywire_lsp/server.py` | Main LSP server logic |
| `tree-sitter-pywire/grammar.js` | Tree-sitter grammar for syntax highlighting |
| `vscode-pywire/` | VSCode extension that uses the LSP |

## Key Components in `server.py`

### `PyWireDocument` Class
Represents a parsed `.pywire` document. Key methods:
...
1. Modify `tree-sitter-pywire/grammar.js`
2. Rebuild the WASM file: `cd tree-sitter-pywire && npx tree-sitter generate && npx tree-sitter build --wasm`
...
LSP logs are written to `/tmp/pywire-lsp.log`. Check this file for errors during development.
